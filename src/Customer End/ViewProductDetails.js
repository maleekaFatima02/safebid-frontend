...
import { Link, Route, useParams, BrowserRouter as Router } from "react-router-dom";
...
export default function ImgMediaCard() {
  ...
  const loadTotalBids = (productId) => {
    displayTotalBids(productId).then(
      data => {
        if (data.error) {
          setError(data.error);
        } else {
  ...
  const loadCurrentBid = (productId) => {
    displayCurrentBid(productId).then(
      data => {
        if (data.error) {
          setError(data.error);
        } else {
                ...
                <Grid xs={12} md={6} lg={8} item>
                  <Card style={{ borderWidth: "1px", borderColor: "#1e3d59" }}>
                    <CardMedia
                      ...
                      height="320"
                      pt="56.25%"
                      src={`${process.env.REACT_APP_SAFE_BID_URI}/product/picture/${product._id}`}
                    />
                    <CardContent>
                      <Typography
                        gutterBottom
                  ...
                  >
                    ...
                    <DialogContent dividers>
                      <Typography gutterBottom>
                        Are you sure you want to place bid on this product?
                      </Typography>
                    </DialogContent>
                    <DialogActions>
